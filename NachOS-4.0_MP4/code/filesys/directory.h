...
public:
    ...
    // TODO begin
    bool Add(char *name, int newSector , bool Dir); // Add file name into the directory
    
    DirectoryEntry* GetTable(){
        return table;
    }

    char* get_idx_name(int idx){
        if(table[idx].inUse == 1 && table[idx].Dir == 1) return table[idx].name;
        return NULL;
    }
    // end
