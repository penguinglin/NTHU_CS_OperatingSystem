...
class Thread {
  ...
  public:
    ...
    Thread(char* debugName, int threadID, int P);    // MP3 for initialization with priority
  ...
  void set_Burst_Time(double T);

  double get_Use_Time();
  void set_Use_Time(double T);

  int get_Wait_Time();
  void set_Wait_Time(int T);

  double get_Accumulate_Use_Time();
  void set_Accumulate_Use_Time(double T);
  ...
  private:
    ...
    // TODO
    int priority;
    double burst_Time, use_Time, accumulate_Use_Time;
    int wait_Time;
...
// TODO
extern int cmp_Priority(Thread *a, Thread *b);
extern int cmp_BurstTime(Thread *a, Thread *b);
